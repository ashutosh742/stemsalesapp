...
class BDRequest_model extends CI_Model {
    ...
    public function detail($id, $requesting_uid) {
        ...
        $req_user = $this->db->query('SELECT uid, role FROM user WHERE uid = ? LIMIT 1', array($requesting_uid))->row_array();
        $role = ($req_user && isset($req_user['role'])) ? strtolower($req_user['role']) : '';
