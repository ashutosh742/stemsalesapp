...
class MomV2_model extends CI_Model {
    ...
    public function get_draft($cid_id, $uid) {
        $this->db->select('id, cmpid_id, cstatus, mainbd, dm_contact_name, dm_contact_designation, dm_contact_phone, dm_contact_email, dm_contact_org_type');
    ...
    public function approval_queue($manager_uid, $cluster = null, $limit = 50) {
        $this->db->select('m.id AS mom_id, m.init_cmpid AS cid_id, m.user_id AS bd_uid, u.username AS bd_name,
                           cm.compname AS school_name, m.meeting_purpose_v2, m.meeting_with,
        ...
        $this->db->join('init_call ic', 'ic.id = m.init_cmpid', 'left');
        $this->db->join('company_master cm', 'cm.id = ic.cmpid_id', 'left');
        $this->db->join('user u', 'u.uid = m.user_id', 'left');
        ...
        if ($cluster) $this->db->where('ic.cluster_id', $cluster);
