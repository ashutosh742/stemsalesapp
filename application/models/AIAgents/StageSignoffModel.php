...
class StageSignoff_model extends CI_Model {
    ...
    public function sweep_stuck_alarms() {
        $now = time();
        $rows = $this->db->where('status', 'pending')->get('lead_stage_signoff')->result_array();
        $stats = ['pushed_4h' => 0, 'emailed_24h' => 0, 'auto_escalated' => 0];

        foreach ($rows as $s) {
            $age = ($now - strtotime($s['requested_at'])) / 3600;
            $in_business_window = $this->_in_business_hours($s['requested_at']);
    ...
    private function _alarm_4h($s) {
        ...
        // The actual in-app push is handled by NotificationDispatcher
        $this->load->library('NotificationDispatcher');
        $this->notificationdispatcher->push_to_rm_of_cm($s['cm_uid'], [
            'title' => 'Signoff stuck 4h',
            'body'  => 'BD signoff request on lead ' . $s['init_call_id'] . ' gate ' . $s['gate_code'] . ' waiting 4h with CM.',
        ]);
    }
    ...
    private function _alarm_24h($s) {
        ...
        $cm = $s['cm_uid'] ? $this->db->select('fname,email')->where('uid', $s['cm_uid'])->get('user')->row_array() : null;
        $rm_email = $this->_rm_email_for_cm($s['cm_uid']);
        $recipients = ['stemlearning@gmail.com'];
        if ($rm_email) $recipients[] = $rm_email;
        $subject = 'Signoff over 24h - lead ' . $s['init_call_id'] . ' gate ' . $s['gate_code'];
        $body  = "CM " . ($cm['fname'] ?? 'unassigned') . " has a signoff request open for over 24 hours.\n";
        $body .= "Gate: " . $s['gate_code'] . "\n";
        $body .= "Lead id: " . $s['init_call_id'] . "\n";
        $body .= "Requested at: " . $s['requested_at'] . "\n";
    ...
    public function queue_for_cm($cm_uid, $limit = 50) {
        return $this->db->order_by('requested_at', 'ASC')
            ->where('cm_uid', $cm_uid)
            ->where('status', 'pending')
            ->limit($limit)
