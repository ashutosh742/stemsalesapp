...
class LineManagerScorecard_model extends CI_Model {
    ...
    public function team_scorecard($rm_uid, $from = null, $to = null) {
        ...
        $cm_rows = $this->db->select('uid, name, type_id')
            ->from('user')
            ->where('cluster_master_id', $cluster)
    ...
    public function leaderboard($from = null, $to = null, $limit = 50) {
        ...
        if (!$to)   $to   = date('Y-m-d');
        $sql = "
            SELECT s.manager_uid, s.manager_role, s.cluster_id, u.name AS manager_name,
            ...
            GROUP BY s.manager_uid, s.manager_role, s.cluster_id, u.name
    ...
    private function _load_manager($uid) {
        $row = $this->db->select('uid, type_id, cluster_master_id, name')
            ->where('uid', $uid)
            ->get('user')
            ->row_array();
        if (!$row) return null;
        ...
        return [
            ...
            'cluster_id' => $row['cluster_master_id'] ? (int)$row['cluster_master_id'] : null,
            'fname' => $row['name'],
        ];
    }

    private function _cluster_for_rm($rm_uid) {
        $r = $this->db->select('cluster_master_id AS cluster_id')->where('uid', $rm_uid)->get('user')->row_array();
