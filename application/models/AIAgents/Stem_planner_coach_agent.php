...
class PlannerCoachAgent_model extends CI_Model
{
    ...
    public function compute_live_suggestions($bd_uid, $plan_date = null)
    {
        ...
        $cluster_sql = "
            SELECT u.cluster FROM user u WHERE u.uid = ? LIMIT 1
        ";
    ...
    public function compute_discipline_report($plan_date = null)
    {
        ...
        $sql = "
            SELECT dp.uid AS bd_uid,
                   ...
                   MAX(CASE WHEN dp.is_same_day_plan = 1 THEN 1 ELSE 0 END) AS same_day_flag
    ...
    public function compute_execution_live($plan_date = null)
    {
        ...
        $sql = "
            SELECT dp.uid AS bd_uid, COUNT(*) tasks_planned
            FROM daily_planner dp
            JOIN daymanagementapprovalrequest a ON a.plan_id = dp.id AND a.status = 'approved'
            WHERE dp.plan_date = ?
            GROUP BY dp.uid
        ";
        $rows = $this->db->query($sql, array($plan_date))->result_array();
    ...
    private function count_skip_no_cancel($bd_uid, $plan_date)
    {
        ...
        $sql = "
            SELECT COUNT(*) c
            FROM daily_planner dp
            LEFT JOIN tblcallevents e ON e.uid = dp.uid AND e.cid_id = dp.cid_id AND DATE(e.event_date) = dp.plan_date
            WHERE dp.uid = ? AND dp.plan_date = ?
              AND e.id IS NULL
              AND dp.cancellation_status IS NULL
              AND TIMESTAMPDIFF(MINUTE, CONCAT(dp.plan_date, ' ', dp.plan_time), NOW()) > 30
        ";
        $r = $this->db->query($sql, array($bd_uid, $plan_date))->row_array();
        return (int)(isset($r['c']) ? $r['c'] : 0);
    }
