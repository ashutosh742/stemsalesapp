...
class CmPlanner_model extends CI_Model {
    ...
    public function joint_meetings_auto_pull($cm_uid, $plan_date) {
        // Use tblcallevents (has user_id + cid_id). daily_planner lacks these columns.
        // Team scope: user_details.aadmin = cm_uid. Also include CM's own events.
        $cm_uid = (int)$cm_uid;
        $sql = "SELECT te.id AS planner_id, te.user_id AS bd_uid, te.cid_id AS lead_id,
                       te.actiontype_id,
                       TIME(te.appointmentdatetime) AS starting_time,
                       NULL AS ending_time,
                       cmp.compname AS school_name, cmp.compname AS compny_nm,
                       ic.cstatus,
                       u.name AS bd_first, '' AS bd_last
                FROM tblcallevents te
                JOIN user u ON u.uid = te.user_id
                JOIN user_details ud ON ud.user_id = te.user_id
                JOIN init_call ic ON ic.id = te.cid_id
                LEFT JOIN company_master cmp ON cmp.id = ic.cmpid_id
                WHERE DATE(te.appointmentdatetime) = ?
                  AND te.actiontype_id IN (3, 4)
                  AND ic.cstatus IN (8, 9, 12)
                  AND (ud.aadmin = ? OR te.user_id = ?)
                ORDER BY te.appointmentdatetime ASC
                LIMIT 100";
        return $this->db->query($sql, array($plan_date, $cm_uid, $cm_uid))->result_array();
    }
    ...
    public function missed_mandatory_yesterday() {
        ...
        $sql = "
            SELECT l.expected_cm_uid AS cm_uid, l.lead_id, l.bd_uid, l.cstatus_at_meeting,
                   ...
                   cm.compname AS school_name, cm.compname AS compny_nm,
                   uc.name AS cm_first, '' AS cm_last,
                   ub.name AS bd_first, '' AS bd_last
            ...
            ORDER BY l.cstatus_at_meeting DESC, l.expected_cm_uid
        ";
        return $this->db->query($sql, array($yest))->result_array();
    }
    /* -------------------------------------------------------------------------
     * Get CM's own leads (init_call.mainbd = cm_uid) with today's planned tasks.
     * Supports the "CM has their own funnel" use case.
     * ---------------------------------------------------------------------- */
    public function get_own_leads($cm_uid, $date = null) {
        $date = $date ?: date('Y-m-d');
        $sql = "SELECT ic.id AS lead_id, ic.cstatus, ic.mainbd AS bd_uid,
                       cmp.compname AS school_name, cmp.id AS company_id,
                       te.id AS event_id,
                       TIME(te.appointmentdatetime) AS starting_time,
                       te.actiontype_id,
                       ? AS plan_date
                ...
                LEFT JOIN tblcallevents te ON te.cid_id = ic.id
                    AND te.user_id = ic.mainbd
                    AND DATE(te.appointmentdatetime) = ?
                WHERE ic.mainbd = ?
                ORDER BY ic.cstatus DESC, te.appointmentdatetime ASC
                LIMIT 100";
        return $this->db->query($sql, array($date, $date, (int)$cm_uid))->result_array();
    }
