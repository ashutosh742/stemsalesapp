...
class AILeadScore_model extends CI_Model {
    ...
    public function score_bd($bd_uid, $score_date = null) {
        ...
        $leads = $this->db
            ->select('ic.id, ic.cstatus, ic.fbudget, ic.createDate, ic.mainbd')
            ->from('init_call ic')
    ...
    private function extract_features($lead) {
        ...
        // Meetings in last 30 days (tblcallevents uses `date` col, not event_date)
        ...
        // Days since last touch (tblcallevents `date` col)
        $row = $this->db->select_max('date')
            ->where('cid_id', $cid)
            ->get('tblcallevents')->row();
        $f['days_since_last_touch'] = $row && $row->date
            ? (int) ((time() - strtotime($row->date)) / 86400)
            : 999;

        // MoM approved in window (mom_data uses init_cmpid not cid_id, cdate not created_at)
        $f['approved_moms_30d'] = (int) $this->db
            ->where('init_cmpid', $cid)
            ->where('approved_status', 1)
            ->where('cdate >=', date('Y-m-d', strtotime('-30 days')))
            ->count_all_results('mom_data');

        // Sentiment: mom_sentiment table not present on staging -- default zeros
        $f['latest_sentiment'] = 0.0;
        $f['buying_signal'] = 0;
        $f['blocker_signal'] = 0;
