...
class RmUpsell_model extends CI_Model {
    ...
    public function pipeline_for_rm($rm_uid) {
        ...
        $sql = "
            SELECT
                p.id, p.lead_id, p.category_code, p.upsell_stage, p.proposal_budget_rs AS fbudget_rs,
                ...
                (SELECT compname FROM company_master WHERE id=ic.cmpid_id) AS school_name, (SELECT locations FROM company_master WHERE id=ic.cmpid_id) AS location,
                u.name AS bd_first, NULL AS bd_last, u.uid AS bd_uid
    ...
    public function pipeline_by_category($rm_uid, $category_code) {
        ...
        $sql = "
            SELECT
                ...
                DATEDIFF(NOW(), p.last_rm_touch_at) AS days_since_rm_touch,
                (SELECT compname FROM company_master WHERE id=ic.cmpid_id) AS school_name, (SELECT locations FROM company_master WHERE id=ic.cmpid_id) AS location,
                u.name AS bd_first, NULL AS bd_last
            FROM rm_upsell_pipeline p
            INNER JOIN init_call ic ON ic.id = p.lead_id
            LEFT JOIN user u ON u.uid = ic.mainbd
            WHERE p.rm_uid = ?
    ...
    public function anchor_renewals_due($rm_uid = null, $within_days = 60) {
        ...
        $where = "p.category_code='ANCHOR' AND p.anchor_renewal_due_date IS NOT NULL AND p.anchor_renewal_due_date <= DATE_ADD(CURDATE(), INTERVAL ? DAY)";
        if ($rm_uid !== null) {
            $where .= " AND p.rm_uid = ?";
            $params[] = (int)$rm_uid;
        }
        $sql = "
            SELECT
                p.id, p.lead_id, p.rm_uid, p.anchor_renewal_due_date AS renewal_due_date, p.proposal_budget_rs AS fbudget_rs,
                p.current_cstatus AS upsell_stage, p.last_rm_touch_at,
                DATEDIFF(p.anchor_renewal_due_date, CURDATE()) AS days_to_renewal,
                p.school_name, p.compny_loction AS location,
                ur.name AS rm_name
            FROM rm_upsell_pipeline p
            LEFT JOIN user ur ON ur.uid = p.rm_uid
            WHERE $where
            ORDER BY p.anchor_renewal_due_date ASC
        ";
    ...
    public function rm_scorecard($rm_uid, $date_yyyymmdd) {
        ...
        $sql_k14 = "
            SELECT COUNT(DISTINCT (SELECT locations FROM company_master WHERE id=ic.cmpid_id)) AS districts_touched
        ...
        $sql_k15 = "
            ...
            WHERE rm_uid = ?
              ...
              AND anchor_renewal_due_date BETWEEN DATE_SUB(?, INTERVAL 90 DAY) AND ?
        ";
        ...
        $sql_k16 = "
            SELECT
                SUM(CASE WHEN rl.rm_led=1 THEN rl.contract_value_rs ELSE 0 END) AS rm_led_rs,
                SUM(rl.contract_value_rs) AS total_rs
            ...
            WHERE act.category_code IN ('PSU','DMFT','ANCHOR')
              AND rl.won_at >= DATE_SUB(?, INTERVAL 90 DAY)
