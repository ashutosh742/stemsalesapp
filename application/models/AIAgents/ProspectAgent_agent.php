...
class Prospect_model extends CI_Model {
    ...
    public function score_bd($bd_uid, $score_date = null) {
        ...
        $research = (int)$this->db->query(
            "SELECT COUNT(*) c FROM tblcallevents
             WHERE uid = ? AND actiontype_id = ? AND purpose_id = ?
               AND DATE(event_date) = ?",
        ...
        $leads = $this->db->query(
            "SELECT category_code FROM init_call
        ...
        foreach ($leads as $r) {
            $c = $r['category_code'] ?: 'cold';
            if (isset($by_cat[$c])) $by_cat[$c]++;
        }
    ...
    public function score_all($score_date = null) {
        ...
        $bds = $this->db->query(
            "SELECT DISTINCT u.uid FROM user u
             ...
             WHERE ud.type_id IN (1) AND u.is_active = 1"  // type_id 1 = BD
        )->result_array();
        $out = [];
    ...
    public function suggest_for_area($bd_uid, $area_name, $city = 'Mumbai',
        ...
        foreach ($web_results as $w) {
            // dedupe against existing init_call by name+area
            $dup = $this->db->query(
    ...
    public function recent_runs_by_bd($bd_uid, $days = 7) {
        return $this->db->query(
            "SELECT * FROM location_prospect_run
