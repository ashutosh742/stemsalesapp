...
class Stem_prospecting_discipline_scorer extends CI_Model
{
    ...
    {
        // rimlyproof_dayshapefix_20260609: aligned to real schema.
        $flags = [];

        // Day start ceremony: daymanagementapprovalrequest(USER_ID, CREATED_AT, STATUS='Approved')
        $started = $this->db
        ...
        // Plan-window submit for tomorrow: daily_planner(userID, record_date, planner_approvel_status)
        $tomorrow = date('Y-m-d', strtotime("$audit_date +1 day"));
        $plan_submitted = $this->db
            ->from('daily_planner')
            ->where('userID', $bd_uid)
            ->where('record_date', $tomorrow)
            ->count_all_results();
        if ($plan_submitted === 0) $flags[] = 'plan_late';

        // Out-of-band events (uses event time band on the pulled events)
        $out_of_band = 0;
        foreach ($bd_events as $ev) {
        ...
        // Seed gap: location_prospect_suggestion(bd_uid, status, seed_date)
        if ($this->db->table_exists('location_prospect_suggestion')) {
            $seed_row = $this->db
                ->select("SUM(CASE WHEN seed_date IS NOT NULL THEN 1 ELSE 0 END) AS seeded,
        ...
        // Seed execution gap is not tracked in the current daily_planner schema
        // (no seeded_from_prospect / task_completed columns); skipped intentionally.
    ...
    {
        // rimlyproof_scorerfix_20260609: real table is leave_requests (date-range + multi-stage status).
        $wfh = $this->db
            ->from('leave_requests')
            ->where('user_id', $bd_uid)
            ->where('start_date <=', $audit_date)
            ->where('end_date >=', $audit_date)
            ->where_in('status', ['approved_admin', 'approved_manager'])
            ->limit(1)
            ->get()->row();
        if ($wfh) return 'leave';
        return null;
    ...
    private function _pull_meeting_events($audit_date)
    {
        ...
        return $this->db->query("
            SELECT
              e.id, e.user_id AS mainbd, e.cid_id, e.event_date, e.gps_reading_time AS event_time,
              e.actiontype_id, e.purpose_id,
              NULL AS photo_url, e.photo_size_kb, e.photo_phash,
              e.gps_lat, e.gps_lng, e.gps_accuracy_meters, e.gps_reading_time,
              ic.school_lat, ic.school_lng,
              u.cluster_master_id AS cluster_id
            ...
            ORDER BY e.user_id, e.gps_reading_time
        ", [$audit_date])->result();
    }
    ...
    private function _pull_active_bds($audit_date)
    {
        ...
        return $this->db->query("
            SELECT uid, name, cluster_master_id AS cluster_id
            FROM user
            WHERE type_id = 2
              AND active = 1
              AND (DATE(created_at) <= ? OR created_at IS NULL)
        ", [$audit_date])->result();
    }
