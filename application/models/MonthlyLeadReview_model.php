...
class MonthlyLeadReview_model extends CI_Model
{
     ...
     * Eligible: active leads (cstatus NOT IN 12,13,14) OR closed within 6 months.
     */
    ...
    private function _eligible_leads($month)
    {
        ...
        $sql = "
            SELECT ic.id, cm.compname AS compny_name, ic.cstatus, ic.fbudget,
                   ic.mainbd AS bd_uid, ic.createDate, 1 AS is_active,
                   ic.updated_at AS last_stage_change_at, u.cluster_id
            FROM init_call ic
            LEFT JOIN company_master cm ON cm.id = ic.cmpid_id
            JOIN user u ON u.uid = ic.mainbd
            WHERE ic.cstatus NOT IN (12, 13, 14)
               OR (ic.cstatus IN (12, 13) AND ic.updated_at >= ?)
    ...
    {
        $sql = "SELECT mlr.*, cm.compname AS school_name, ic.cstatus AS cstatus_now
    ...
    {
        $sql = "SELECT mlr.*, cm.compname AS school_name, ic.cstatus AS cstatus_now,
                       u.name AS bd_first_name, '' AS bd_last_name
                FROM monthly_lead_review mlr
                JOIN init_call ic ON ic.id = mlr.lead_id
                LEFT JOIN company_master cm ON cm.id = ic.cmpid_id
    ...
    {
        $sql = "SELECT mlr.*, cm.compname AS school_name,
                       u.name AS bd_first_name, '' AS bd_last_name,
                       uc.name AS cm_first_name, '' AS cm_last_name
                FROM monthly_lead_review mlr
                JOIN init_call ic ON ic.id = mlr.lead_id
                LEFT JOIN company_master cm ON cm.id = ic.cmpid_id
                JOIN user u ON u.uid = mlr.bd_uid
