...
WHERE
    ...
    GROUP BY u1.zone_id");
     ...
     public function GetBDFunnelsDatas_zoneID($zone_id){
         ...
         $query2 = $this->db->query("SELECT
            ...
            u1.name as bd_name,
            init_call_bkp.id as init_id,
            init_call_bkp.cmpid_id as cid,
            init_call_bkp.cstatus,
            init_call_bkp.lstatus,
            init_call_bkp.clm_id,
            init_call_bkp.acm_co_id,
            init_call_bkp.apst,
            init_call_bkp.rm_east_co_id,
            init_call_bkp.rm_north_co_id,
            init_call_bkp.ash_nae_co_id,
            init_call_bkp.ash_w_co_id
    ...
    public function GetBDFunnelsStatusDatasByZoneID($zone_id){
        ...
        $query = $this->db->query("SELECT
            ...
            FROM
                ...
                LEFT JOIN user_details u1 on u1.user_id = init_call_bkp.mainbd
                LEFT JOIN user_zone on user_zone.id = u1.zone_id
                LEFT JOIN status on status.id = init_call_bkp.cstatus
            WHERE
                u1.status = 'active'
                AND u1.zone_id = $zone_id
                GROUP BY status.id");
    ...
    public function CheckPrimaryContactExistsOrNotinTable($cid){
        $query  = $this->db->query("SELECT * FROM `company_contact_master` WHERE company_id = '$cid' AND type IN ('primary', 'primary')");
        $result = $query->result();
        return $result;
    }
