...
class Handover_v2_model extends CI_Model {
    ...
    public function list_for_cm_approval($cm_uid) {
        ...
        $sql = 'SELECT vp.* FROM v_handover_pending vp '
             . 'LEFT JOIN user ub ON ub.uid = vp.closing_bd_uid '
             . 'WHERE vp.status = ? AND (vp.cm_uid = ? OR ub.reporting_cm_uid = ?) '
    ...
    public function detail($id, $requesting_uid) {
        ...
        $req_user = $this->db->query('SELECT uid, role, reporting_cm_uid FROM user WHERE uid = ? LIMIT 1', array($requesting_uid))->row_array();
        if (!$req_user) {
            return array('ok' => false, 'error' => 'Requesting user not found');
        }
        $role = isset($req_user['role']) ? strtolower($req_user['role']) : '';
        ...
        if ($role === 'admin') {
            ...
            $bd = $this->db->query('SELECT reporting_cm_uid FROM user WHERE uid = ? LIMIT 1', array((int)$row['closing_bd_uid']))->row_array();
            if ($bd && (int)$bd['reporting_cm_uid'] === $requesting_uid) {
                $allowed = true;
            }
