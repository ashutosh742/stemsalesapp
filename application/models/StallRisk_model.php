...
class StallRisk_model extends CI_Model
{
    ...
    public function run_nightly_batch($scope = 'pilot')
    {
        ...
        foreach ($leads as $lead) {
            try {
                ...
                $this->upsert_score($result, $run_id, ($scope === 'pilot') ? 1 : 0);

                $stats['scored']++;
                $bucket_key = strtolower($result['bucket']);
                if ($bucket_key === 'at_risk') $bucket_key = 'at_risk';
                if (isset($stats[$bucket_key])) {
                    $stats[$bucket_key]++;
                }
            } catch (Exception $e) {
                $stats['errors']++;
    ...
    public function score_lead_by_cid($cid_id)
    {
        ...
        // Schema fix 2026-06-06: init_call PK is 'id' not 'cid_id'; column names corrected
        // Using raw query to avoid CI3 query builder escaping NULL keyword
        $lead = $this->db->query(
            'SELECT ic.id AS cid_id, ic.mainbd AS bd_uid, ic.cmpid_id AS cm_uid, ic.cstatus, ic.fbudget,
                    NULL AS partner_hint,
                    IF(ic.dm_contact_filled_at IS NOT NULL,1,0) AS dm_contact_complete,
                    ic.exschool AS school_name, ic.updated_at AS cstatus_updated_at
             FROM init_call ic
    ...
    public function get_stall_aging_by_cm()
    {
        $sql = "
            SELECT
                s.cm_uid,
                CONCAT(u.first_name, ' ', u.last_name) AS cm_name,
            ...
            WHERE DATE(s.computed_at) = CURDATE()
              AND s.cm_uid > 0
            GROUP BY s.cm_uid, u.first_name, u.last_name
    ...
    {
        $q = $this->db->select('ic.id AS cid_id, ic.mainbd AS bd_uid, ic.clm_id AS cm_uid, ic.cstatus, ic.fbudget, NULL AS partner_hint, NULL AS dm_contact_complete, cm.compname AS school_name, ic.updated_at AS cstatus_updated_at')
                      ->from('init_call ic')
