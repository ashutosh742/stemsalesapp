...
class CompanyDetail_api extends CI_Controller {
    ...
    private function _bd_may_view($ids) {
        // type_id 3 = Sales Person (BD)
        if ($this->_authed_type !== 3) return true;
        $uid = $this->_authed_uid;
        if ($uid <= 0) return true;
        // BD may view if they are mainbd on ANY init_call row for this company
        // (match by company_master.id OR by the specific init_call.id)
        $r = $this->db->query(
