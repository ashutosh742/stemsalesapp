...
class Pst extends RestApiBaseController {
    ...
    public function unassigned() {
        $sql = "SELECT ic.id AS cid_id, cm.compname AS school_name, cm.city, ic.program, ic.apst, ic.pstadt,
                    ic.num_of_proposal, ic.fbudget, ic.cstatus,
                    CONCAT(u.fname, ' ', u.lname) AS bd_name
                FROM init_call ic
                LEFT JOIN company_master cm ON cm.id = ic.cmpid_id
                LEFT JOIN users u ON u.id = ic.mainbd
                WHERE ic.cstatus = 7
                  AND (ic.apst IS NULL OR ic.apst = '' OR ic.apst = 0)
                ORDER BY ic.pstadt ASC LIMIT 200";
        $res = $this->db->query($sql);
        $leads = $res ? $res->result_array() : array();

        $pst_sql = "SELECT u.id AS uid, CONCAT(u.fname, ' ', u.lname) AS name,
                           COUNT(ic2.id) AS open_count
                    FROM users u
                    LEFT JOIN init_call ic2 ON ic2.apst = u.id AND ic2.cstatus = 7
                    WHERE u.type_id IN (2, 13, 3)
                    GROUP BY u.id, u.fname, u.lname ORDER BY open_count ASC LIMIT 50";
    ...
    public function assign() {
        ...
        $updated = 0;
        foreach ($lead_ids as $cid_id) {
            $cid_id = (int)$cid_id;
            if ($cid_id <= 0) continue;
            $this->db->query(
                "UPDATE init_call SET apst = ?, pstadt = ? WHERE cid_id = ? AND (apst IS NULL OR apst = '' OR apst = 0)",
                array($pst_id, $due_date, $cid_id)
            );
            $updated++;
        }
    ...
    public function change() {
        ...
        $old = $this->db->query(
            "SELECT apst FROM init_call WHERE cid_id = ? LIMIT 1",
            array($lead_id)
        )->row_array();
        $old_pst = $old ? (int)$old['apst'] : 0;

        $this->db->query("UPDATE init_call SET apst = ? WHERE cid_id = ?", array($new_pst_id, $lead_id));
    ...
    public function conversions() {
        ...
        $sql = "SELECT ic.id AS cid_id, cm.compname AS school_name, cm.city, ic.program, ic.fbudget,
                    ic.apst, ic.pstadt, ic.cstatus,
                    CONCAT(up.fname, ' ', up.lname) AS pst_name,
                    CONCAT(ub.fname, ' ', ub.lname) AS bd_name,
                    ftl.transfer_date AS conversion_date
                FROM init_call ic
                LEFT JOIN company_master cm ON cm.id = ic.cmpid_id
                LEFT JOIN funnel_transfer_log ftl
                    ON ftl.cid_id = ic.id AND ftl.status_to = 12
                    AND DATE(ftl.transfer_date) BETWEEN ? AND ?
                LEFT JOIN users up ON up.id = ic.apst
                LEFT JOIN users ub ON ub.id = ic.mainbd
                WHERE ic.cstatus = 12 AND ic.apst > 0
                  AND DATE(ftl.transfer_date) BETWEEN ? AND ?
                ORDER BY ftl.transfer_date DESC LIMIT 300";

        $rows = $this->db->query($sql, array($from, $to, $from, $to))->result_array();
        $total_rs = array_sum(array_column($rows, 'fbudget'));
