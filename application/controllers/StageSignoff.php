...
class StageSignoff_api extends CI_Controller {
    ...
    public function request_signoff() {
        ...
        foreach ($required as $f) {
            if (empty($in[$f])) {
                ...
                echo json_encode(array('error' => 'missing field ' . $f));
                return;
            }
        }
    ...
    public function decide() {
        ...
        if (!in_array($decision, array('approve', 'reject'), true)) {
            http_response_code(400);
            echo json_encode(array('error' => 'decision must be approve or reject'));
            return;
        }
    ...
    public function bypass() {
        ...
        if (strlen(trim($in['reason'])) < 10) {
            http_response_code(400);
            echo json_encode(array('error' => 'reason must be at least 10 characters'));
            return;
        }
    ...
    public function queue() {
        ...
        if (empty($cm_uid) && empty($rm_uid)) {
            http_response_code(400);
            echo json_encode(array('error' => 'cm_uid or rm_uid required'));
            return;
        }
    ...
    public function pending_for_bd() {
        ...
        if (empty($bd_uid)) {
            http_response_code(400);
            echo json_encode(array('error' => 'bd_uid required'));
            return;
        }
