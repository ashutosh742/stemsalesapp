 ...
 *
 * Endpoints:
 *   GET /api/anaya/briefing?uid=<uid>
 *     Returns the live morning briefing for one BD.
 *
 *   GET /api/anaya/leads_to_push?uid=<uid>
 *     Returns up to 10 leads that most need a touch today.
 *
 *   GET /api/anaya/probe
 *     Returns { ok:true, deployed:true }.
 *
 * Never fabricates. All counts come from init_call and tblcallevents.
 */
class AnayaBriefing extends CI_Controller {
    ...
    public function briefing() {
        ...
        try {
            $r = $this->db->select('COUNT(DISTINCT company_id) n', false)
                ->from('init_call')->where('mainbd', $uid)->get()->row();
    ...
    private function _leads_to_push($uid, $limit = 5) {
        $cutoff = date('Y-m-d 00:00:00', strtotime('-14 days'));
        try {
            $sql = "SELECT ic.cid AS cid_id, ic.cname AS school, ic.cstatus,
                           (SELECT MAX(te.appointmentdatetime) FROM tblcallevents te
                              WHERE te.cid_id = ic.cid) AS last_touch
                    FROM init_call ic
                    WHERE ic.mainbd = ?
                      AND ic.cstatus NOT IN (12,13,14)
