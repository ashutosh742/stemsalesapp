 ...
 * Migration 024.
 */
class FunnelHygieneController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->load->model('AIAgents/FunnelHygiene_model', 'hygiene');
    ...
    private function _jwt_token_valid($token) {
        ...
        $candidates = array();
        foreach (array('uid','cm_uid','rm_uid','bd_uid','acm_uid','user_id','manager_uid') as $k) {
    ...
    public function no_purpose()
    {
        ...
        $rows = $this->db->query("
            SELECT n.*, cm.compname AS school_name, u.name AS bd_name
    ...
    public function phantom()
    {
        $days = (int)($this->input->get('days') ?: 7);
        $rows = $this->db->query("
            SELECT p.*, cm.compname AS school_name, u.name AS bd_name
    ...
    public function weekly_gap()
    {
        ...
        $sql = "SELECT w.*, cm.compname AS school_name, u.name AS bd_name
    ...
    public function stagnant_22()
    {
        ...
        $sql = "SELECT s.*, cm.compname AS school_name,
                       u.name AS bd_name, ic.fbudget AS pipeline_rs
    ...
    public function dm_queue()
    {
        ...
        $rows = $this->db->query("
            SELECT d.*, cm.compname AS school_name, u.name AS bd_name
    ...
    public function dm_run_batch()
    {
        ...
        $this->load->model('AIAgents/DmVerifyAgent_model', 'dm_agent');
    ...
    {
        if ($this->input->method() !== 'post') $this->_json(['error' => 'post_only'], 405);
        $this->load->model('AIAgents/LineManagerScorecard_v2_K8_patch', 'scorecard');
