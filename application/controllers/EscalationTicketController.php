...
class EscalationTicket_api extends CI_Controller {
    ...
    private function _auth() {
        ...
        // Root-cause fix 20260608: use the same Bearer_auth library every working agent uses,
        // instead of comparing against an undefined STEM_DIGEST_TOKEN constant (which rejected all).
        $this->load->library('Bearer_auth');
        $tok = $this->bearer_auth->get_bearer_token();
        if (!$this->bearer_auth->verify($tok)) {
