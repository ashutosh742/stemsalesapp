...
class LineManagerScorecardController extends CI_Controller {
    ...
    public function __construct() {
        ...
        if (!stem_auth_bearer_ok($this->input->get_request_header('Authorization'))) {
            // rimlyproof_ctorreturn_20260609: constructor return; does NOT stop CI
            // from running the requested method (fail-open). Emit 401 and exit.
    ...
    public function scorecard() {
        ...
        if (!$uid) {
            ...
            $this->output->set_content_type('application/json');
            $this->output->set_output(json_encode(['ok' => true, 'rows' => [], 'note' => 'no_data', 'detail' => 'uid_required']));
            return;
        }
        $out = $this->sm->scorecard($uid, $from, $to);
        $this->output->set_output(json_encode($out));
        } catch (Exception $e) {
            log_message('error', 'LineManagerScorecardController::scorecard: ' . $e->getMessage());
            header('Content-Type: application/json');
    ...
    public function leaderboard() {
        ...
        $this->output->set_output(json_encode([
            ...
            log_message('error', 'LineManagerScorecardController::leaderboard: ' . $e->getMessage());
            header('Content-Type: application/json');
            http_response_code(200);
            echo json_encode(['ok' => true, 'rows' => [], 'note' => 'no_data', 'detail' => $e->getMessage()]);
        }
    ...
    public function refresh() {
        ...
        $this->output->set_output(json_encode(['date' => $date, 'result' => $out]));
            ...
            log_message('error', 'LineManagerScorecardController::refresh: ' . $e->getMessage());
            header('Content-Type: application/json');
            http_response_code(200);
            echo json_encode(['ok' => true, 'rows' => [], 'note' => 'no_data', 'detail' => $e->getMessage()]);
        }
